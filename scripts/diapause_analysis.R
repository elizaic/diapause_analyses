...
diapauseData2 <- diapauseData %>%
  ...
  select(-notes)  %>% 
  mutate(time_raw = (end_date - start_date)) %>%
  ...
  mutate(time_raw = as.numeric(time_raw, units = 'days'))
...
diapauseData3 <- left_join(diapauseData2, 
  ...
  mutate(
    #  new columns for days to diapause and proportion in diapause for 
    # 1) the survival model (time_surv), which
        # includes all the individuals, but those not diapausing at the end are set to 43
    # 2) for all other analyses (time_qc), which
        # keeps 43 set for those not diapausing, but replaces all other non-diapausers with NA
        # which excludes those that died during the experiment from being included as diapausing
    # 3) for the proportion in diapause analyses, which removes dead individuals from being 
        # counted as not in diapause
...
diapauseData3 %>% group_by(treatment, core_edge) %>% summarise(
  mean = mean(time_qc, na.rm = TRUE),
  sd = sd(time_qc, na.rm = TRUE),
  n = n()
)
...
modDiapausing_pois <- glmmTMB(time_qc ~ core_edge * treatment + (1| population), data = diapauseDataDiapausing,
                         family = poisson())
...
modDiapausing <- glmmTMB(time_qc ~ core_edge * treatment + (1| population), data = diapauseDataDiapausing,
                         family = nbinom2())
...
modDiapausingpop <- glmmTMB(time_qc ~  treatment * population, data = diapauseDataDiapausing,
                            family = nbinom2)
...
diapausers_reaction_plot_new <- ggplot(data = meansDiapausing_df, 
  ...
  # add data points
  geom_point(data = diapauseDataDiapausing, aes(x = treatment, y = time_qc, color = core_edge),
  ...
  #sample size
  geom_text(aes(label = dtd_n1, y = -1.5), 
...
## summary statistics
summ_dtd_all_pop <- diapauseData3 %>% 
  group_by(treatment, population) %>% 
  filter(!is.na(time_qc)) %>% # need to filter out the dead indivs to get accurate n()
  summarise(
  mean = mean(time_qc, na.rm = TRUE),
  sd = sd(time_qc, na.rm = TRUE),
  n = n()
...
# summ_dtd_all_pop <- summ_dtd_all_pop %>% mutate(n2 = replace(n, n==24, 'n=24') )

summ_dtd_all <- diapauseData3 %>% 
  group_by(treatment, core_edge) %>% 
  filter(!is.na(time_qc)) %>% # need to filter out the dead indivs to get accurate n()
  summarise(
    mean = mean(time_qc, na.rm = TRUE),
    sd = sd(time_qc, na.rm = TRUE),
...
diapauseData3 %>%
  summarise(mean = mean(time_qc, na.rm = TRUE),
            variance = var(time_qc, na.rm = TRUE))

## model with core_Edge, pop as rand
# Poisson model is over dispersed
modAllSamp_pois <- glmmTMB(time_qc ~ core_edge * treatment + (1| population), data = diapauseData3,
                      family = poisson())
simulateResiduals(modAllSamp_pois) %>% plot()
testDispersion(modAllSamp_pois)



# negative binomial model to fix over dispersion
modAllSamp <- glmmTMB(time_qc ~ core_edge * treatment + (1| population), data = diapauseData3,
                      family = nbinom1())
...
modAllSamppop <- glmmTMB(time_qc ~  treatment * population, data = diapauseData3,
                         family = nbinom1())
...
days_plotnew <- ggplot(data = emoutmodAllSamp_df, aes(x = treatment, y = response, color = core_edge)) +
  ...
  # non-diapauser label
  annotate("text", x = c(1.55), y = 43.3, label = "non-diapausers", 
           size = 3.1) +
  # p-value labels
  annotate("text", label = plot_pval_labs(modAllSamp), 
           x = 0.5, y = 50, size = 3, hjust = 0, vjust = 1) +
  # add data points
  geom_point(data = diapauseData3, aes(x = treatment, y = time_qc, color = core_edge),
             position = position_jitterdodge(dodge.width = 0.3, jitter.width = 0.2),
             alpha = 0.1) +
  #population lines and points
  ...
  #sample size
  geom_text(data = summ_dtd_all, aes(label = n, y = -1.5), 
...
survObj <- Surv(diapauseData3$time_surv, diapauseData3$diapause)

# sample size - core/edge
summ_dtd_all_surv <- diapauseData3 %>% 
  group_by(treatment, core_edge) %>% 
  summarise(
    mean = mean(time_surv, na.rm = TRUE),
    sd = sd(time_surv, na.rm = TRUE),
    n = n()
  )

# sample size - populations
summ_dtd_all_pop_surv <- diapauseData3 %>% 
  group_by(treatment, population) %>% 
  summarise(
    mean = mean(time_surv, na.rm = TRUE),
    sd = sd(time_surv, na.rm = TRUE),
...
survreg_means_df <- survreg_means_df %>% 
  left_join(., summ_dtd_all_pop_surv %>% 
              select(treatment, population, n))
...
surv_plot_new <- ggplot(data = survreg_means_CE_df, aes(x = treatment, y = emmean, color = core_edge)) +
  ...
  annotate("text", x = c(1.55), y = 43.3, label = "non-diapausers",
           size = 3.1) +
  # add data points
  geom_point(data = diapauseData3, aes(x = treatment, y = time_surv, color = core_edge),
             position = position_jitterdodge(dodge.width = 0.3, jitter.width = 0.2),
             alpha = 0.1) +
  #population lines and points
  ...
  geom_pointrange(aes(x = treatment, y = emmean, ymin = lower.CL, ymax = upper.CL),
                  position = position_dodge(width = 0.3), size = .7) +
  #sample size
  geom_text(data = summ_dtd_all_surv, aes(label = n, y = -5), 
            size = 2.6,
            position = position_dodge(width = 0.35),
            show.legend = FALSE) +
  #formatting
...
ggarrange(surv_plot_new, surv_pop_plot_new, common.legend = FALSE,
          legend = 'right', labels = "AUTO", widths = c(0.9, 1)) %>%
...
# diapusePropSummaryPop <- 
  ...
  summarise(
    n = n(),
    prop = mean(diapause_qc, na.rm = TRUE),
    sd = sd(diapause_qc, na.rm = TRUE),
    se = sd/sqrt(n),
    latitude = mean(latitude)
  )
# diapusePropSummaryPop

diapauseData3 %>% group_by(treatment, core_edge) %>% 
  summarise(
  mean_time = mean(time_qc, na.rm = TRUE),
  prop = mean(diapause_qc, na.rm = TRUE),
  sd_prop = sd(diapause_qc, na.rm = TRUE),
  n = n()
)

##Sample size 
prop_sample_size <- diapauseData3 %>% 
  group_by(population, treatment) %>% 
  filter(!is.na(diapause_qc)) %>%
  summarize (
  n = n()
)
...
# #Model core/edge
# logmodel1 <- glmmTMB(diapause_qc ~ core_edge * treatment + (1|population), 
...
library(brms)
library(tidybayes)
logistic_ce_brm <- brm(diapause_qc ~ core_edge * treatment + (1|population), 
          family = bernoulli(), 
...
# population means
logistic_means_pop_df <- diapauseData3 %>%
  distinct(population, treatment, core_edge) %>%
  add_epred_draws(logistic_ce_brm, re_formula = NULL) %>%  # Include random effects
  mean_qi(.epred) %>%
  unite("treatment_core_edge", c(treatment, core_edge), remove = FALSE) %>%
  rename(prob = .epred,
         lower.CL = .lower,
...
logistic_means_pop_df <- logistic_means_pop_df %>% mutate(letter = rep(c("A", "B", "C", "D", "E", "F", "G", "H"), each = 2))
logistic_means_pop_df <- logistic_means_pop_df %>%
...
# logmodel_bayes <- arm::bayesglm(diapause_qc ~ population * treatment, 
#                                 family = 'binomial', data = diapauseData3)
# vif(logmodel_bayes)
# 
...
# New version of prop in diapause plot
prop_plotnew <- ggplot(data = logistic_means_ce_brm_df, aes(x = treatment, y = response)) +
  #population lines and points
  geom_line(data = logistic_means_pop_df,
            aes(x = treatment, y = prob,
                group = population, color = core_edge),
            position = position_dodge(width = 0.4),
            alpha = 0.3) +
  geom_pointrange(data = logistic_means_pop_df,
                  aes(x = treatment, y = prob, ymin = lower.CL, ymax = upper.CL,
                      group = population, color = core_edge),
                  position = position_dodge(width = 0.4),
                  alpha = 0.3, shape = 15) +
  ...
  geom_line(aes(color = core_edge, group = core_edge), 
            position = position_dodge(width = 0.3)) +
  geom_pointrange(aes(x = treatment, y = response, ymin = lower.HPD, ymax = upper.HPD, 
                      color = core_edge),
                  position = position_dodge(width = 0.3), size = 0.7) +
  #sample size
  geom_text(data = summ_dtd_all, aes(label = n, y = 0, color = core_edge), 
            position = position_dodge(width = 0.4),
            show.legend = FALSE) +
  #formatting
...
prop_pop_new <- ggplot(data = logistic_means_pop_df) +
  ...
  ggpattern::scale_pattern_manual(values = c("none", "stripe"),
                                  labels = c('long' = "Long", 'short' = "Short")) + # Define pattern types
  geom_errorbar(data = logistic_means_pop_df,
                aes(x = treatment, ymin = lower.CL, ymax = upper.CL,
                    group = population), width = 0.9,
                position = position_dodge2(width = 2),
                color = 'black') +
  #sample size
  geom_text(data = logistic_means_pop_df,
...
prop_days_pop <- full_join(logistic_means_pop_df,emoutAllSampPops_df, by = c('treatment', 'population'))
corr_diapause <- cor.test(prop_days_pop$prob, prop_days_pop$response, method = c("pearson"))

ggplot(data = prop_days_pop)+
  geom_smooth(aes(x = prob, y = response), method = lm, color = 'black') +
  geom_point(aes(x = prob, y = response, fill = core_edge.x, shape = treatment),
              size = 3) +
...
cor.test(prop_days_pop_long$prob , prop_days_pop_long$response , method = c("pearson"))

prop_days_pop_short <- prop_days_pop %>% filter(treatment == 'short')
cor.test(prop_days_pop_short$prob , prop_days_pop_short$response , method = c("pearson"))
...
library(lme4)
library(lmerTest)
mod_PC1_all <- lmer(time_qc ~ PC1 * treatment + (1|population), 
...
ggplot(data = diapauseData_pc1, 
       aes(x = PC1, y = time_qc, group = treatment)) +
